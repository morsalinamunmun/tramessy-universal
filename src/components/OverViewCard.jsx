...
const OverViewCard = () => {
  ...
  // Fetch daily expenses from payments API
  useEffect(() => {
    const fetchDailyExpenses = async () => {
      try {
        ...
        const todayPayments = payments.filter(
          payment => dayjs(payment.date).format("YYYY-MM-DD") === today
        )
  ...
  useEffect(() => {
    const fetchTripData = async () => {
      try {
        ...
        const totalTripCost = todayTrips.reduce(
          (sum, trip) => sum + Number.parseFloat(trip.total_exp || 0), 
          0
        )
        setTotalTripExpenses(totalTripCost)
        setTodayIncome(totalTripIncome)
  ...
  const totalFuelCost = octenCost + dieselCost + petrolCost + gasCost
  const totalExpense = totalFuelCost + todayMaintenanceCost + totalTripExpenses + dailyExpenses
  const profit = todayIncome - totalExpense
  ...
  const expenseTableData = [
    ...
    },
    {
      ...
      status: dailyExpenses > 0 ? "active" : "inactive",
    },
  ]

  // Columns for Today's Net Profit table
  const todayNetProfitColumns = [
    {
      title: "Income",
      dataIndex: "income",
  ...
  // Data for Today's Net Profit table
  const todayNetProfitData = [
    {
      key: "today-profit",
      income: todayIncome,
      fuelExpense: totalFuelCost,
    ...
    <div className="p-6">
      <Row gutter={[16, 16]} className="mb-5">
        ...
        <Col xs={24} sm={24} md={12} lg={8}>
          <Card
            ...
            bodyStyle={{ padding: "12px", maxHeight: "150px", overflowY: "auto" }}
          >
            {/* ... existing parts reminder content ... */}
          </Card>
        </Col>
        {/* document reminder */}
        <Col xs={24} sm={12} md={12} lg={8}>
          <Card
            ...
            bodyStyle={{ padding: "12px", maxHeight: "150px", overflowY: "auto" }}
          >
            {/* ... existing document reminder content ... */}
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} lg={12}>
          ...
          >
            <Table
              loading={loadingExpenses || loadingDailyExpenses}
              dataSource={expenseTableData}
              columns={[
                {
                  title: "Expense Type",
                ...
                },
                {
                  ...
                  render: (status) => (
                    <Tag color={status === "active" ? "red" : "default"}>
                      {status === "active" ? "Expense" : "Not Expense"}
                    </Tag>
                  ),
                },
              ]}
        ...
        <Col xs={24} lg={12}>
          ...
          >
            <Table
              loading={loadingIncome || loadingExpenses || loadingTrips || loadingDailyExpenses}
              dataSource={todayNetProfitData}
              columns={todayNetProfitColumns}
              pagination={false}
              size="small"
